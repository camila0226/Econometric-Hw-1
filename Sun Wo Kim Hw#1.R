...
#/Users/jaydenkim/Desktop/Econometrics   #my working directory 
Dice.exp <- read.csv("/Users/jaydenkim/Desktop/Econometrics/SunWoKim_DiceExperiment.csv")


#plot for dice.exp


#/Users/jaydenkim/Desktop/Econometrics
list.files(path = '/Users/jaydenkim/Desktop/Econometrics'  
, full.names = TRUE)
...
Dice.exp_long <- data.frame(
  Frequency = c(Dice.exp$Frequency.of.6.control., Dice.exp$Frequency.of.6.Experiment.),
  Group = rep(c("Control", "Experiment"), each = nrow(Dice.exp))
)

# Plot the combined histogram with facet_wrap
ggplot(Dice.exp_long, aes(x = Frequency, fill = Group)) +
...
ggplot(Dice.exp, aes(x = as.factor(Frequency.of.6.control.))) +
...
ggplot(Dice.exp, aes(x = as.factor(Frequency.of.6.Experiment.))) +
...
#'In the first 10 rolls, I did nothing but roll two standard six-sided dice as a control group.
#'For the second set of 10 rolls, I modified the dice by shaving and slightly rounding the angles on the face of the 6.
#'Additionally, I added gum to the back of the 1 face to further increase the likelihood of landing on a 6.
#'Results showed that the modified dice in the second set produced more 6s compared to the unaltered dice in the control group.
...
#load("/Users/jaydenkim/Desktop/Econometrics/Sun Wo Kim-Dice Experiment)
...
#When analyzing the impact of educational attainment on employment, it's intriguing to see that a significant portion of individuals with advanced degrees are married.
#How does marital status influence career advancement, and what role does educational attainment play in this dynamic?

#I find it noteworthy that a large percentage of respondents in this survey reported experiencing anxiety several days in the past two weeks.
#What might be the long-term effects of such prevalent anxiety on workforce productivity and overall societal well-being?
